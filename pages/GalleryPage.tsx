...
const MOCK_ITEMS: GalleryItem[] = [
    {
      id: 1001,
      url: 'https://cdn.coverr.co/videos/coverr-walking-by-the-sea-4623/1080p.mp4',
      caption: 'Ocean Vibes (Demo)',
      type: 'video'
    },
    ...Array.from({ length: 5 }).map((_, i) => ({
      id: 2000 + i,
      url: `https://picsum.photos/600/${Math.floor(Math.random() * 400 + 400)}?random=${i}`,
      caption: `Demo Memory ${i + 1}`,
      type: 'image' as const
    }))
];
...
export const GalleryPage: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this memory?")) {
        if (id > 10000) {
            // It's a user item (timestamp based ID)
            await deleteItemFromDB(id);
            const dbItems = await getItemsFromDB(decodedMonth);
            setItems(dbItems); // Will be empty array if last item deleted
        } else {
            // It's a mock item
